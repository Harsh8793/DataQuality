...
CHAT_PLANNER_SYSTEM = (
    ...
    "History shows each past turn's actual SQL on a '[ran this SQL]' line. For a follow-up, "
    "START FROM THAT SQL and change ONLY what the user just asked for:\n"
    "  · 'as a pie chart' / 'show me a graph' → same SQL, set \"chart\"; do NOT touch the "
    "WHERE, GROUP BY or the measure.\n"
    "  · 'top 3' / 'only 5' → same SQL, adjust ORDER BY + LIMIT.\n"
    "  · 'by state instead' / 'break it down by product' → same measure and same filters, "
    "REPLACE the grouped dimension with the newly named column (GROUP BY and the first "
    "selected column both change).\n"
    "  · 'for 2024' / 'just Texas' → same SQL, ADD that filter.\n"
    "NEVER invent a WHERE clause that is absent from the previous SQL and unnamed in the new "
    "message. In particular, do not turn a value that merely appeared in your own previous "
    "sentence into a filter: if the last answer read 'total revenue in North America is $2.8M' "
    "and the previous SQL had no WHERE, then 'now as a pie chart' must also have NO WHERE.\n"
    "If the previous SQL grouped by a column and the follow-up names a DIFFERENT column, that "
    "is a dimension swap, not an extra filter.\n"
...
CHAT_NARRATE_SYSTEM = (
    ...
    "- Every number you write must appear in the result or in the 'Verified "
    "facts' block. Never estimate, round beyond two decimals, or work out a "
    "total or percentage yourself — if one matters, it is given to you.\n"
    "- When verified facts are supplied, write the finding rather than a list: "
    "name the leader with its share of the total and contrast it with the "
    "lowest. Prefer 'X leads with 753,660.34 of the 2,899,305.58 total (26.0%), "
    "ahead of Y; Z is lowest at 267,691.84' over reciting rows.\n"
